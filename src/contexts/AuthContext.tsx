...
// Define types for authentication events
interface AuthEvent {
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const initializeAuth = async () => {
    try {
      ...
      if (session?.user) {
        ...
        // If no Supabase session, check for stored user data
        console.log('No Supabase session found, checking stored data');
        const userJson = await SecureStore.getItemAsync('user');
        const sessionData = await SecureStore.getItemAsync('userSession');
        
        if (userJson && sessionData) {
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      if (data.user && data.session) {
        console.log('Sign in successful with Supabase');
  ...
  const createUserProfile = async (supabaseUser: any, username?: string) => {
    try {
